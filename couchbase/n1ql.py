...
#     def __init__(self, ks, kv):
#         # Build the statement first:
#         ss = 'INSERT INTO {0} (PRIMARY KEY, VALUE) VALUES {1}'
#         keys = kv.keys()
#         placeholders = []
#         for x in range(len(keys)):
#             placeholders.append('(?,?)')
#         ss = ss.format(ks, ','.join(placeholders))
#         super(N1QLInsertQuery, self).__init__(ss)
#
#         batch_args = []
#         for k in keys:
#             batch_args.append([k, kv[k]])
#         print batch_args
#         self.set_option('batch_args', json.dumps(batch_args))
