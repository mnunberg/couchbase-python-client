...
enum {
    ...
    /* Must exist! */
    RESTYPE_MUST_EXIST = (1 << 5) | RESTYPE_EXISTS_OK
};
...

{
    ...
    *res = (pycbc_Result*)PyDict_GetItem(mrdict, hkey);
    ...
    if (*res == NULL) {
        if (restype & RESTYPE_MUST_EXIST) {
            PYCBC_EXC_WRAP(PYCBC_EXC_INTERNAL, 0, "Missing Result object in callback!");
            return -1;
        }
...
subdoc_callback(lcb_t instance, int cbtype, const lcb_RESPBASE *rb)
{
    ...
    rv = get_common_objects(rb, &conn, (pycbc_Result**)&res, RESTYPE_MUST_EXIST, &mres);
    if (rv < 0) {
        goto GT_DONE;
    }
